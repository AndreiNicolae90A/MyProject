...
export const create = (user: User, callback: Function) => {
  const queryString =
    "INSERT INTO jsusers (nume, prenume, poza, email, datanastere, telefon, cetatenie) VALUES (?, ?, ?, ?, ?, ?, ?)";
    console.log(user);
  db.query(
    queryString,
    [user.nume, user.prenume, user.poza, user.email, user.datanastere, user.telefon, user.cetatenie],
    (err, result) => {
      if (err) {
        callback(err);
      }
...
export const update = (user: User, callback: Function) => {
  ...
  db.query(queryString, [user.nume, user.prenume, user.poza, user.email, user.datanastere, user.telefon, user.cetatenie, user.id], (err, result) => {
    if (err) {
      callback(err);
    }
    callback(null);
